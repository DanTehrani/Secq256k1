...
impl WideScalar {
    ...
    pub fn mul_wide(a: &Scalar, b: &Scalar) -> Self {
        let a = a.0.as_uint().to_words();
        let b = b.0.as_uint().to_words();
    ...
    pub(crate) fn mul_shift_vartime(a: &Scalar, b: &Scalar, shift: usize) -> Scalar {
        ...
        let res = Scalar(ScalarCore::new(U256::from_words([r0, r1, r2, r3])).unwrap());
    ...
    fn reduce_impl(&self, modulus_minus_one: bool) -> Scalar {
        ...
        Scalar(ScalarCore::new(U256::conditional_select(&r, &r2, !underflow | high_bit)).unwrap())
    }
